...
import { updateZip, updateSqlFile } from '@/api/data';

// 状态变量
const sqlVersion = ref('1.0.0');
const zipVersion = ref('1.0.0');
...
const uploadSql = async ({ file, onProgress }) => {
    try {
        ...
        sqlVersion.value = result.version;
        sqlLastSync.value = new Date().toLocaleString('zh-CN');
...
const uploadZip = async ({ file, onProgress }) => {
    try {
        ...
        const result = await response.json();
        zipVersion.value = result.version;
        zipLastSync.value = new Date().toLocaleString('zh-CN');
